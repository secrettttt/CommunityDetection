...
def load_data():
    ...
    fileIn.close()
    points = np.array(points)#把列表变成数组
    print(points)
    return points
...
if __name__ == '__main__':
    ...
    clusterRes, clusterNum = dbscan(data, 0.5 ,5)#这里改变DBSCAN的超参数
#    clusterRes, clusterNum = dbscan(data, 1 ,5)#这里改变DBSCAN的超参数
#    clusterRes, clusterNum = dbscan(data, 1 ,2.5)#这里改变DBSCAN的超参数
    cluster_DBSCAN=list(clusterRes)
    print(len(cluster_DBSCAN))
    print('clusterNumber=',clusterNum)
    
    #把聚类结果写入Clust_DBSCAN.csv中
    b = []
    ...
    dataframe.to_csv("Clust_DBSCAN.csv",index=False,sep=',')
